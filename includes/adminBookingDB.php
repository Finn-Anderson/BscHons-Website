<?php
	...
	try {
		...
		$dayCapacity->bindValue(":froms", ucfirst($from), PDO::PARAM_STR);
		$dayCapacity->bindValue(":to", ucfirst($to), PDO::PARAM_STR);
		...
		$avgSeasonCapacity = $conn->prepare("SELECT AVG(numberOfPeople) as 'avgNum' FROM Booking, Trip, RouteFare, Route WHERE Booking.bookingID = Trip.bookingID AND Trip.routeFareID = RouteFare.routeFareID AND RouteFare.routeID = Route.routeID AND Route.from = :froms AND Route.to = :to AND reverse = :reverse");
		$avgSeasonCapacity->bindValue(":froms", ucfirst($from), PDO::PARAM_STR);
		$avgSeasonCapacity->bindValue(":to", ucfirst($to), PDO::PARAM_STR);
